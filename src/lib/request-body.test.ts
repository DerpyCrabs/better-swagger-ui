...
describe('primaryTryItOutMedia', () => {
  it('prefers json over text/plain', () => {
    const info = getRequestBodySchema(spec, postOp('/mixed')!.requestBody)!
    expect(primaryTryItOutMedia(info)?.contentType).toBe('application/json')
  })

  it('selects text/plain when json is absent', () => {
    const info = getRequestBodySchema(spec, postOp('/notes')!.requestBody)!
    expect(primaryTryItOutMedia(info)?.contentType).toBe('text/plain')
  })

  it('selects multipart/form-data', () => {
    const info = getRequestBodySchema(spec, postOp('/upload')!.requestBody)!
    expect(primaryTryItOutMedia(info)?.contentType).toBe('multipart/form-data')
  })
})

describe('getRequestBodyMode', () => {
  it('detects json, text, file, multipart, and urlencoded modes', () => {
    const mixed = primaryTryItOutMedia(getRequestBodySchema(spec, postOp('/mixed')!.requestBody)!)
    const notes = primaryTryItOutMedia(getRequestBodySchema(spec, postOp('/notes')!.requestBody)!)
    const binary = primaryTryItOutMedia(getRequestBodySchema(spec, postOp('/binary')!.requestBody)!)
    const upload = primaryTryItOutMedia(getRequestBodySchema(spec, postOp('/upload')!.requestBody)!)
    const form = primaryTryItOutMedia(getRequestBodySchema(spec, postOp('/form')!.requestBody)!)
...
describe('defaultRequestBodyText', () => {
  it('uses string example for text/plain', () => {
    const media = primaryTryItOutMedia(getRequestBodySchema(spec, postOp('/notes')!.requestBody)!)
...
describe('resolveMultipartFields', () => {
  it('marks binary properties as file fields', () => {
    const media = primaryTryItOutMedia(getRequestBodySchema(spec, postOp('/upload')!.requestBody)!)
...
describe('buildRequestBody', () => {
  ...
  it('builds urlencoded form body', () => {
    const media = primaryTryItOutMedia(getRequestBodySchema(spec, postOp('/form')!.requestBody)!)
